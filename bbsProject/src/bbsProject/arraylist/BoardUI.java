...
public class BoardUI {
	...
	private List<String> registerPost() {
		...
		while(true) {
			...
			switch(input) {
				...
				case "1":
					...
					List<String> post = boardJob.retrieveDetail(sc.nextLine());
					viewPostDetail(post);
					break;
				case "2":
					System.out.print("# 게시글 번호: ");
					String no = sc.nextLine();	
					List<String> newPost = registerPost();
	...
	private void viewPosts(List<List<String>> posts) {
		System.out.println(">> " + curBoard.getNameKr() + " 목록을 조회합니다.");

		// header
		String str = "", line = "";
		List<String> fields = Arrays.asList(curBoard.getFields());
		...
		for (List<String> p : posts) {
			System.out.println(getFormattedPost(p));
		}
	}
	
	private void viewPostDetail(List<String> post) {
		System.out.printf("%25.25s\n", "no." + post.get(0) + " post " + "~".repeat(25));
		for(String p : post) {
	...
	private String getFormattedPost(List<String> post) {
		String str = "";

		str += MyFormatter.getFormatStr(post.get(0), 10) + " "; // no
		str += MyFormatter.getFormatStr(post.get(1), 30) + " "; // title
		str += MyFormatter.getFormatStr(post.get(3), 20) + " "; // name
		str += MyFormatter.getFormatStr(post.get(5), 20) + " "; // regDate

		if (post.size() > 6) {
			str += MyFormatter.getFormatStr(post.get(6), 10) + " "; // 추가 필드
		}
